      ...
      >
        <div className="flex items-center justify-between mb-4 bg-green-50 p-2 rounded-lg">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-primary flex items-center justify-center text-[10px] text-white font-bold">₦</div>
            <span className="text-xs font-bold">Wallet</span>
          </div>
          <span className="text-xs font-bold">750,000.00</span>
        </div>
        ...
        <div className="flex items-center justify-between bg-gray-50 p-2 rounded-lg mt-1">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full overflow-hidden border border-gray-200">
               <img src="https://i.pravatar.cc/100?u=jane" className="w-full h-full object-cover" alt="Recipient" />
            </div>
            <span className="text-xs font-bold">Jane Doe</span>
          </div>
          <span className="text-xs font-bold text-primary">₦50,000.00</span>
        </div>
      </motion.div>
    )
  },
...
export const OnboardingScreen: React.FC<{ onComplete: () => void, onLogin: () => void }> = ({ onComplete, onLogin }) => {
  ...
  return (
    <div className={`fixed inset-0 flex flex-col transition-colors duration-500 overflow-hidden ${step.isDark ? 'bg-primary' : 'bg-green-50/30'}`}>
      <BrandPattern opacity={0.03} color={step.isDark ? 'white' : 'primary'} size={60} />
      {/* Progress Bar */}

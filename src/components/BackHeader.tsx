...
export const BackHeader = ({ title, subtitle, onBack, theme = 'light', onHome, className }: { title: string, subtitle?: string, onBack?: () => void, theme?: 'light' | 'dark', onHome?: () => void, className?: string }) => (
  <header className={`px-4 py-4 sticky top-0 z-20 flex flex-col w-full max-w-full box-border overflow-x-hidden ${theme === 'dark' ? 'bg-gray-900' : 'bg-green-50/30'} ${className || ''}`}>
